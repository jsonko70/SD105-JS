...
router.post('/search', function (req, res, next) {
    ...
    list = titanic.filter(p => p.name.includes(search));  //          TODO --- your code goes here

    //  TODO --- count the living and the dead
    let alive = list.reduce((total, t) => total + (t.survivor  ? 1 : 0), 0);       //  count the survivors
    let dead  = list.reduce((total, t) => total + (t.survivor  ? 0 : 1), 0);       //  count the dead
...
router.post('/fancy', function (req, res, next) {
    let fancy = req.body.fancy;        //  TODO --- where will you find the search text from the browser?
    let list;      
    let fancySearch = fancy.split(':');         
    //  SEARCH OPTION IDEAS
    ...
    switch (fancySearch[0]) {
            case "underage": list = titanic.filter(p => p.age < fancySearch[1]); break;
            case "overage": list = titanic.filter(p => p.age > fancySearch[1]); break;
            case "name": list = titanic.filter(p => p.name.includes(fancySearch[1])); break;
            case "crew": list = titanic.filter(p => p.passengerCrew.includes('Crew')); break;
            case "passengers": list = titanic.filter(p => p.passengerCrew.includes('Passenger')); break;
            case "class": list = titanic.filter(p => p.passengerClass.includes(fancySearch[1])); break;
            case "role": list = titanic.filter(p => p.role.includes(fancySearch[1])); break;       
    }
